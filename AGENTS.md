...
10. **Example calls are pre-recorded, honestly framed.** The two example buttons play `apps/web/public/example-calls/<scenario>.mp3` and emit the matching `<scenario>.json` transcript through the same SSE channel as live calls. The UI labels them as samples; don't dress them up as live. Generated by `pnpm example-calls:build` (`scripts/example-calls.ts`) using OpenAI's `gpt-4o-mini-tts` model. The live agent in `apps/voice/` uses ElevenLabs Flash v2.5 (free-tier API keys often work for light demo traffic; see `apps/voice/README.md` §Env for credits + licensing). Offline sample MP3s can move to ElevenLabs when the REST build path works for your key—the swap is documented at the top of `scripts/example-calls.ts`.
...
2. `OPENAI_API_KEY=... pnpm example-calls:build` — writes `apps/web/public/example-calls/<slug>.mp3` and `<slug>.json`.
